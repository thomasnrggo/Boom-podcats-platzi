  ...
  render() {
    ...
    return (
      <div className="clip">
        <nav>
          {onClose ?
            <a onClick={onClose}><FontAwesomeIcon icon={faChevronCircleLeft} size="2x" color="#fff" /></a>
            :
            <Link route='channel'
              params={{ slug: slug(clip.channel.title), id: clip.channel.id }}
              prefetch>
              <a className='close'><FontAwesomeIcon icon={faChevronCircleLeft} size="2x" color="#fff" /></a>
            </Link>
          }
        </nav>
